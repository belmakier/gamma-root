#ifndef GAMRSORT_TIMEDIFFERENCEPAIRS_HH
#define GAMRSORT_TIMEDIFFERENCEPAIRS_HH
...
public:
   TimeDifferencePairs(Sorter *parent, std::vector<std::string> opts) : p(parent)
   {
      if (opts.size() < 8)
         ...
         for (auto &deti : vecDets) {
            for (auto &detj : vecDets) {
               ...
               if ((pkgated || bggated) && (deti.template Get<T>() > 0) && (detj.template Get<T>() > 0)) {
                  // tdiff = T2 - T1 ALWAYS.
                  auto tdiff = detj.template GetCal<T>() - deti.template GetCal<T>(); // uncalibrated, think, document
                  if (pkgated) {
   ...
   }             // Sorter::TimeDifferencePairs::operator()
};
